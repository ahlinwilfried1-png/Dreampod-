...
import { History, ArrowLeft, Calendar, TrendingUp } from "lucide-react";
...
type FilterType = "all" | "income" | "deposit" | "withdrawal" | "bonus" | "investment";

export default function HistoryView({ transactions, onBack, userPhone }: HistoryViewProps) {
  ...
  const filteredTransactions = transactions.filter((t) => {
    ...
    if (filter === "income") {
      return t.type === "income" || (t.method && t.method.toLowerCase().includes("revenu"));
    }
    if (filter === "bonus") {
      return t.type === "bonus" || t.type === "commission";
    }
    return t.type === filter;
  });

  const getStatusBadge = (t: Transaction) => {
    if (t.type === "income" || (t.method && t.method.toLowerCase().includes("revenu"))) {
      return (
        <span className="text-[10px] font-black bg-emerald-50 text-emerald-600 border border-emerald-200/50 px-2.5 py-0.5 rounded-full flex items-center gap-1">
          <TrendingUp className="h-3 w-3" /> Revenu Versé
        </span>
      );
    }
    switch (t.status) {
  ...
  const getTransactionLabel = (t: Transaction) => {
    ...
    switch (t.type) {
      ...
      case "income":
        return "Revenu d'investissement";
      case "commission":
    ...
    <div id="history-view-container" className="space-y-6">
      ...
      <div className="flex items-center gap-3.5">
        ...
        <div>
          ...
          <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Suivi en temps réel de votre solde et revenus</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl p-2 border border-slate-200/60 shadow-xs flex gap-1.5 overflow-x-auto select-none no-scrollbar">
        {[
          ...
          { id: "bonus", label: "Bonus & Commissions" },
          { id: "investment", label: "Achats VIP" },
        ].map((tab) => (
          <button
            id={`filter-tab-${tab.id}`}
            key={tab.id}
            onClick={() => setFilter(tab.id as FilterType)}
            className={`text-[10.5px] font-black py-2 px-3.5 rounded-xl cursor-pointer shrink-0 transition-all ${
              filter === tab.id
        ...
        {filteredTransactions.length === 0 ? (
          ...
          <div className="space-y-3">
            {filteredTransactions.map((t) => {
              const isPositive = ["deposit", "commission", "bonus", "income"].includes(t.type) || (t.method && t.method.toLowerCase().includes("revenu"));
              return (
                <div
                  id={`tx-item-${t.id}`}
                  key={t.id}
                ...
                >
                  <div className="flex items-center gap-3.5 max-w-[65%]">
                    <span className="text-2xl h-11 w-11 shrink-0 rounded-xl bg-white flex items-center justify-center shadow-2xs border border-slate-100">
                      {getTransactionIcon(t)}
                    </span>
                    <div className="overflow-hidden">
                      <h4 className="font-extrabold text-slate-800 text-[11.5px] tracking-wide truncate">
